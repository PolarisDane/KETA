...
class TrainLoop:
    def __init__(self, args, train_platform, model, diffusion, data):
        ...
        print(f"self.num_steps: {self.num_steps}", file=sys.stderr)
    ...
    def run_loop(self):
        ...
        loss_list = []
        print(f"self.num_epochs: {self.num_epochs}", file=sys.stderr)
        for epoch in range(self.num_epochs):
            print(f'Starting epoch {epoch}')
            current_loss_list = []
            for motions, cond in tqdm(self.data):
                if not (not self.lr_anneal_steps or self.step + self.resume_step < self.lr_anneal_steps):
                    break

                motion = motions[0].to(self.device)
                joint = motions[1].to(self.device)

                cond['y'] = {key: val.to(self.device) if torch.is_tensor(val) else val for key, val in cond['y'].items()}

                # for k, v in logger.get_current().dumpkvs().items():
                #     if k == 'loss':
                #         current_loss_list.append(v)

                loss = self.run_step(motion, cond)
                current_loss_list.append(loss)
                ...
                if self.step % self.save_interval == 0:
                    ...
                    if os.environ.get("DIFFUSION_TRAINING_TEST", "") and self.step > 0:
                        return
                self.step += 1
            loss_list.append(np.mean(current_loss_list))
            # if epoch % 2 == 0:
                # save an 2D figure of the loss curve for all the epochs up to now from loss_list
            plt.figure()
            plt.plot(range(1, len(loss_list) + 1), loss_list, marker='o', color='b', label='Loss Curve')
            plt.xlabel('Epoch')
            plt.ylabel('Loss')
            plt.title('Loss Curve after' + str(epoch) + 'epochs')
            plt.legend()
            plt.grid(True)
