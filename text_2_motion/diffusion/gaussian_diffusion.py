...
import yaml
from data_loaders.humanml import data
from diffusion.nn import mean_flat, sum_flat
from diffusion.losses import normal_kl, discretized_gaussian_log_likelihood
from data_loaders.humanml.scripts import motion_process
from diffusion.kp_extract import JOI2KP, recover_from_ric
...
class GaussianDiffusion:
    ...
    def __init__(
        ...
        *,
        ...
        lambda_vel_rcxyz=0.,
        lambda_fc=0.,
        lambda_text_kp_align=0.001,
    ):

        std_path = "/data/qiaojun/jy_cv/cv/text_2_motion/dataset/HumanML3D/Std.npy"
        mean_path = "/data/qiaojun/jy_cv/cv/text_2_motion/dataset/HumanML3D/Mean.npy"
        self.data_std = torch.tensor(np.load(std_path)).cuda()
        self.data_mean = torch.tensor(np.load(mean_path)).cuda()
        ...
        self.lambda_text_kp_align = lambda_text_kp_align # todo: need modify
        print(f"lambda_text_kp_align: {lambda_text_kp_align}")
        
        # get_model_args(args=args,
        # config = Box(yaml.load(open(kargs.get('config', '')), Loader=yaml.FullLoader))
        
        self.kp_extractor = JOI2KP()
        self.use_guide_token = False
        # self.text_kp_align_model = TextKPAlignmentModel(**config.model).to(config.model.device)
        # torch.load(self.text_kp_align_model.state_dict(), "~/jy_cv/cv/text_2_motion/save/my_gaussian_6_all/align_model_250.pth")
        # self.text_kp_align_model.eval()
    ...
    def load_align_model(self, align_model_dir, align_model_ckpt):
        config = Box(yaml.load(open(osp.join(align_model_dir, 'model.yaml')), Loader=yaml.FullLoader))
        self.text_kp_align_model = TextKPAlignmentModel(**config.model).to(config.model.device)
        self.text_kp_align_model.load_state_dict(torch.load(align_model_ckpt))
        for p in self.text_kp_align_model.parameters():
    ...
    def p_mean_variance(
        ...
        if self.model_var_type in [ModelVarType.LEARNED, ModelVarType.LEARNED_RANGE]:
            ...
            model_variance = model_variance.permute((0, 2, 1)).unsqueeze(2)
            model_log_variance = model_log_variance.permute((0, 2, 1)).unsqueeze(2)
        ...
        return {
            "mean": model_mean.squeeze(2).permute((0, 2, 1)),
            "variance": model_variance,
            "log_variance": model_log_variance.squeeze(2).permute((0, 2, 1)),
    ...
    def training_losses(self, model, x_start, t, model_kwargs=None, noise=None, dataset=None):
        ...
        if model_kwargs is None:
            model_kwargs = {}
        if self.use_guide_token:
            n_joints = 22
            x_t_copy = deepcopy(x_t)
            x_t_copy = x_t_copy * self.data_std + self.data_mean
            # import pdb; pdb.set_trace()
            x_t_copy = recover_from_ric(x_t_copy, n_joints)
            # import pdb; pdb.set_trace()
            kps = self.kp_extractor(x_t_copy)
            spt_len = model_kwargs['y']['spt_len']

            kps_proc = []
            for i in range(len(kps)):
                kp = kps[i, :model_kwargs['y']['lengths'][i]-1, :]
                T = len(kp)
                n = spt_len[i]
                if n == 1:
                    lT = T
            ...
            model_kwargs['y']['guide_token'] = self.text_kp_align_model.calc_dist(model_kwargs['y']['spt_encoding'], kps_proc, max(model_kwargs['y']['spt_len']), model_kwargs['y']['spt_len']).squeeze(2).permute(1, 0, 2)
        terms = {}
        ...
        if self.loss_type == LossType.KL or self.loss_type == LossType.RESCALED_KL:
            ...
            # import pdb; pdb.set_trace()
            model_output = model_output.squeeze().permute(0, 2, 1)
            ...
            if self.lambda_text_kp_align > 0.:
                # read the two numpy file to get the data_std and data_mean

                # data_std = data_std.unsqueeze(0).unsqueeze(0)
                # data_mean = data_mean.unsqueeze(0).unsqueeze(0)

                # adding an extra dimension to data_std and data_mean, the dimension value = 

                # transform model_output to joints' space
                # need some data_std and data_mean
                n_joints = 22
                model_output = model_output * self.data_std + self.data_mean
                # import pdb; pdb.set_trace()
                model_output = recover_from_ric(model_output, n_joints)
                # model_output = model_output.view(-1, *model_output.shape[2:])
                # import pdb; pdb.set_trace()
                kps = self.kp_extractor(model_output)
                spt_len = model_kwargs['y']['spt_len']
                ...
                terms["text_kp_align_mse"] = self.text_kp_align_model.forward(model_kwargs['y']['spt_encoding'], kps_proc, max(model_kwargs['y']['spt_len']), model_kwargs['y']['spt_len'])

            # torch.autograd.set_detect_anomaly(True)

            # terms["text_kp_align_mse"].backward()

            # import pdb; pdb.set_trace()
            # print(model_kwargs['y']['caption'])
            terms["loss"] = terms["rot_mse"] + terms.get('vb', 0.) +\
                            (self.lambda_vel * terms.get('vel_mse', 0.)) +\
                            (self.lambda_rcxyz * terms.get('rcxyz_mse', 0.)) + \
                            (self.lambda_fc * terms.get('fc', 0.)) + \
                            (self.lambda_text_kp_align * terms.get('text_kp_align_mse', 0.))
