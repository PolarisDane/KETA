...
def main():
    ...
    iterator = iter(data)
    idxxx = 0
    for _, model_kwargs in iterator:
    # else:
    ...
    #     _, model_kwargs = collate(collate_args)
        ...
        for rep_i in range(args.num_repetitions):
            ...
            sample = sample_fn(
                ...
                (args.batch_size, max_frames, model.njoints),  # BUG FIX
                clip_denoised=False,
            ...
            if model.data_rep == 'hml_vec':
                ...
                sample = sample.view(-1, *sample.shape[1:]).permute(0, 2, 3, 1)
        ...
        npy_path = os.path.join(out_path, f'results-{idxxx}.npy')
        print(f"saving results file to [{npy_path}]")
        np.save(npy_path,
                {'motion': all_motions, 'text': all_text, 'lengths': all_lengths,
                'num_samples': args.num_samples, 'num_repetitions': args.num_repetitions})
        with open(npy_path.replace('.npy', '.txt'), 'a') as fw:
            fw.write('\n'.join(all_text)+'\n')
        with open(npy_path.replace('.npy', '_len.txt'), 'a') as fw:
        ...
        for sample_i in range(args.num_samples):
            ...
            for rep_i in range(args.num_repetitions):
                ...
                save_file = fr"{model_kwargs['y']['text'][sample_i]}".replace(" ", "_").replace(",", "").replace(".", "").replace("/", "")
                save_file = str(idxxx) + " " + save_file[:min(30, len(save_file))] + '.gif'
