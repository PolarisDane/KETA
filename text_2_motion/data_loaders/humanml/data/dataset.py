...
def collate_fn(batch):
    ...
    # batch = [(b[1], b[3], b[4], b[7], b[8], b[9]) for b in batch]
...
class Text2MotionDatasetV2(data.Dataset):
    def __init__(self, opt, mean, std, split_file, w_vectorizer):
        ...
        for name in tqdm(id_list[:]):
            try:
                motion = np.load(pjoin(opt.motion_dir, name + '.npy'))
                joints = np.load(pjoin(opt.joint_dir, name + '.npy')).reshape(-1, 22 * 3)
                kps = np.load(pjoin(opt.kp_dir, name + '.npy'))
                if (len(motion)) < min_motion_len or (len(motion) >= 200):
                    continue
                text_data = []
                flag = False
                encoding_file = np.load(pjoin(opt.encoding_dir, name + '.npz'), allow_pickle=True)
                max_spt_len = 0
                with cs.open(pjoin(opt.text_dir, name + '.txt')) as f:
                    text_file = f.readlines()
                
                for line_idx in range(len(encoding_file['len'])):
                    line = text_file[line_idx]
                    line_split = line.strip().split('#')
                    caption = line_split[0]
                    tokens = line_split[1].split(' ')
                    text_dict = {}
                    f_tag = encoding_file['f_tags'][line_idx]
                    to_tag = encoding_file['e_tags'][line_idx]
                    
                    ori_encoding = encoding_file['ori_encoding'][line_idx]
                    spt_len = encoding_file['len'][line_idx]
                    if spt_len == 0:
                        spt_len = 1
                    spt_encoding = encoding_file['spt_encoding'][line_idx][:spt_len]
                                 
                    text_dict['ori_encoding'] = ori_encoding
                    text_dict['spt_encoding'] = spt_encoding
                    text_dict['spt_len'] = spt_len
                    text_dict['tokens'] = tokens
                    text_dict['caption'] = caption
                    
                    if f_tag == 0.0 and to_tag == 0.0:
                        flag = True
                        max_spt_len = max(max_spt_len, spt_len)
                        text_data.append(text_dict)
                    else:
                        try:
                            n_motion = motion[int(f_tag*20) : int(to_tag*20)]
    ...
    def __getitem__(self, item):
        idx = self.pointer + item
        data = self.data_dict[self.name_list[idx]]
        motion, joints, kps, m_length, text_list, max_spt_len = data['motion'], data['joints'], data['kps'], data['length'], data['text'], data['max_spt_len']
        # Randomly select a caption
        ...
        ori_encoding = text_data['ori_encoding']
        spt_encoding = text_data['spt_encoding']
        spt_len = text_data['spt_len']
        ...
        if m_length < self.max_motion_length:
            ...
            joints = np.concatenate([joints,
                                    np.zeros((self.max_motion_length - m_length, joints.shape[1]))
                                    ], axis=0)
            kps = np.concatenate([kps,
                                    np.zeros((self.max_motion_length - m_length, kps.shape[1]))
                                    ], axis=0)
        # print(word_embeddings.shape, motion.shape)
        # print(tokens)
        return kps, motion, joints, m_length, ori_encoding, spt_len, spt_encoding, data['caption'], caption, '_'.join(tokens)
